...
_SOCKETS = ['LGA1851', 'LGA1700', 'LGA1200', 'LGA1151', 'LGA1150', 'LGA1155', 'LGA1156', 'AM5', 'AM4', 'AM3+', 'AM3']
...
_CHIPSET_TO_SOCKET = {
    ...
    'H410': 'LGA1200', 'H470': 'LGA1200', 'B460': 'LGA1200', 'B560': 'LGA1200',
    'Z490': 'LGA1200', 'Z590': 'LGA1200', 'H510': 'LGA1200',
    'H310': 'LGA1151', 'H370': 'LGA1151', 'B360': 'LGA1151', 'Z370': 'LGA1151', 'Z390': 'LGA1151',
}
...
def get_cpu_tdp(component: Component) -> int:
    return get_int_spec(component, 'TDP', 'Тепловыделение', 'Потребляемая мощность', default=65)


def get_gpu_power_recommended(component: Component) -> int:
    return get_int_spec(component, 'Рекомендуемая мощность БП', 'Потребляемая мощность', 'TDP', default=150)
...
def get_cooler_supported_sockets(component: Component) -> list[str]:
    val = get_spec(component, 'Совместимые сокеты', 'Поддерживаемые сокеты', 'Сокет', 'Socket')
    if val:
...
def _normalize_ff(ff: str) -> str:
    return ff.upper().replace('MICRO-ATX', 'MATX').replace('M-ATX', 'MATX').replace('MINI-ITX', 'ITX')
...
def get_case_supported_form_factors(component: Component) -> list[str]:
    val = get_spec(component, 'Совместимые форм-факторы', 'Форм-факторы плат', 'Поддерживаемые форм-факторы', 'Форм-фактор')
    if val:
        val_upper = val.upper()
